...
function drawChart(isReload) {
	axios.get("logs.json").then(response => {
		...
		document.querySelectorAll("svg").forEach(element => {
			let divWidth =
				element.parentElement.parentElement.parentElement.parentElement.parentElement.offsetWidth * 0.002
			if (divWidth < 0.96) {
...
function drawWindowLog(data) {
	...
	data.forEach(element => {
		...
		div.setAttribute("class", "d-flex justify-content-between gap-2")
...
function drawLightsUsage(data) {
	...
	data.forEach(element => {
		if (element.status == "On")
			sumOn = sumTimes(sumOn, getTimeDifference(convertTime(element.end), convertTime(element.start)))
