...
function checkLights(isLightsOn) {
    document.getElementById("lights-status").innerHTML = isLightsOn
        ? "on"
        : "off"
    document.getElementById("lights-status").style.color = isLightsOn
        ? "green"
        : "red"
    document.getElementById(
        isLightsOn ? "light-on" : "light-off"
    ).style.opacity = "1"
    document.getElementById(
        isLightsOn ? "light-off" : "light-on"
    ).style.opacity = "0"
    const request = {
        content: {
